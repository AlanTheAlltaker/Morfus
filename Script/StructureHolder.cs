...
public class StructureHolder : NetworkBehaviour
{
    ...
    [SerializeField] GameObject CannonWarMachine;
    public bool cWMI = false;
    [SerializeField] GameObject Teleport;
    public bool tI = false;
    [SerializeField] GameObject HealingStation;
    public bool hSI = false;
    [SerializeField] GameObject TokenCollector;
    public bool tCI = false;
    private void Update()
    {
        if (!IsOwner) { return; }
        HideServerRpc(true);
    ...
    void HideClientRpc(bool gunCannonB)
    {
        ...
        if (cWMI)
        {
            CannonWarMachine.SetActive(gunCannonB);
        }
        if (tI)
        {
            Teleport.SetActive(gunCannonB);
        }
        if (hSI)
        {
            HealingStation.SetActive(gunCannonB);
        }
        if (tCI)
        {
            TokenCollector.SetActive(gunCannonB);
        }
    }
}
